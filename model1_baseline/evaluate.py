...
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix # degerlendirme fonksiyonlarinin import edilmesi

from data_prep import get_dataloaders # veri kumelerinin import edilmesi
from model import CatVsJetCNN # cnn modelinin import edilmesi
...
def evaluate_model():
    ...
    model.load_state_dict(torch.load('cat_vs_jet_model1_baseline.pth', weights_only=True)) # Eğitim sırasında kaydedilen model ağırlıklarını yükler.
    model.eval()
    ...
    print("========== TEST SONUCLARI (Model 1 - Baseline) ==========")
    ...
    with open('results/metrics_model1_baseline.txt', 'w', encoding='utf-8') as f:
        f.write("Model 1 - Baseline Test Sonuclari\n")
    ...
    # ============================================
    # 3. ADIM: CONFUSION MATRIX GORSELI 
    # ============================================
    ...
    plt.tight_layout()
    plt.savefig('results/confusion_matrix_model1_baseline.png')
    ...
    history = torch.load('history_model1_baseline.pth', weights_only=False)
    ...
    plt.tight_layout()
    plt.savefig('results/loss_accuracy_curves_model1_baseline.png')
    plt.close()
